...
/**
 * Legacy synchronous populateTypesAndNames. Retained as `populateTypesAndNamesLegacy`
 * so the perf NUT can run it side-by-side with the Effect-based version
 * (see `populateTypesAndNames.ts`) for equivalence + perf comparison.
 *
 * Internal-only export. Will be removed once the Effect version has soaked.
 */
export const populateTypesAndNamesLegacy =
  ({
    projectPath,
    forceIgnore,
