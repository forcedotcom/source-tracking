...
describe('end-to-end-test for local tracking where pkgdir is a symlink', () => {
  ...
  before(async () => {
    ...
    // move the project to a symlink
    const forceAppPath = path.join(session.project.dir, 'force-app');
    await fs.promises.cp(forceAppPath, path.join(session.project.dir, 'actual'), { recursive: true });
    await fs.promises.rm(forceAppPath, { recursive: true });
    await fs.promises.symlink(path.join(session.project.dir, 'actual'), forceAppPath);
  });
  ...
  it('should commit the project with session-relative filenames and a message', async () => {
    ...
    const remaining = await repo.getChangedFilenames();

    expect(remaining).to.be.an('array').with.length(0);
  });
